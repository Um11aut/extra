...
public:
  ...
  Node(const Node &other) : _val(other._val) {
    _children.first = (other._children.first)
                          ? std::make_unique<Node>(*other._children.first)
                          : nullptr;
    _children.second = (other._children.second)
                           ? std::make_unique<Node>(*other._children.second)
                           : nullptr;
  }

  void setFirst(Ptr node) { _children.first = std::move(node); }

  void setSecond(Ptr node) { _children.second = std::move(node); }
...
static bool areEqual(const Node::Ptr &first, const Node::Ptr &second) {
  if (first.get() == nullptr && second.get() == nullptr)
    return true;
