...
template <typename T> class _element {
public:
  using _ptr = std::shared_ptr<_element<T>>;

  _element(T val) : value(val) {}

  void setPrev(_ptr &prev) { _refs.first = prev; }

  void setNext(_ptr &next) { _refs.second = next; }
...
public:
  ...
  linked(U &&...args)
      : _elements({std::forward<_ptr>(_ptr(new _element<U>(args)))...}) {
  ...
  T &operator[](const size_t index) { return _elements[index]->value; }

  _ptr &get(const size_t index) { return _elements[index]; }
  ...
  void dublicateValues() {
    ...
    std::for_each(_elements.begin(), _elements.end(), [&](const auto &el) {
...
template <typename T> static void print(list::linked<T> &list) {
